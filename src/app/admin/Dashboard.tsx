						...
						{data?.map((blog, index) => (
							...
							<Link to={`/blog/${blog?.id}`} key={index} >
								<div className='flex group py-5 gap-8 justify-between flex-row-reverse md:w-full transition-colors duration-200 cursor-pointer ease-in-out md:border-b border-gray-200'>
									<img src={blog?.imageUrl} loading='lazy' className=' min-w-[100px] h-[60px] md:min-w-[160px] md:h-[107px] border object-cover rounded-xl md:rounded-2xl' alt="" />
									<div className='flex  flex-col  gap-3 w-full '>
										...
										<div className='md:flex items-center hidden justify-between mt-2 w-full '>
											...
											<div className='flex items-center gap-4'>
												<img src={blog?.authorImg || ''} loading='eager' className={`size-6 md:size-10 object-cover rounded-full cursor-pointer`} alt="" />
